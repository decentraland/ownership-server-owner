...
if [ "$DOCKER_TAG" != "latest" ]; then
    echo -e "\033[33m WARNING: You are not running latest image of Ownership Server. \033[39m"
fi

echo -n " - SERVER_URL:                " ; echo -e "\033[33m ${SERVER_URL} \033[39m"
...
docker pull "quay.io/decentraland/ownership-server:${DOCKER_TAG:-latest}"
if [ $? -ne 0 ]; then
  echo -n "Failed to pull the ownership server's docker image with tag ${DOCKER_TAG:-latest}"
  printMessage failed
  exit 1
fi
...
export nginx_url="$(echo "${SERVER_URL##*/}")"
